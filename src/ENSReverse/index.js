...
export default () => {
  ...
  const [name, setName] = useState("Are there /'s Allowed? 🐳")
  ...
  const handlers = [
    ...
    },
    {
      ...
      func: async () => {
        ...
        setAddr(addr = (await window.ethereum.enable())[0])
...
//    },
  ...
  return (
    <div>
      <table>
        <tr><th>Your Address:</th><td>{addr}</td></tr>
        <tr><th>Current Network:</th><td>{net}</td></tr>
      </table>

      <div style={{display: 'flex', flexDirection: 'column'}}>
        {handlers.map(h => (
          <button onClick={h.func}>{h.name}</button>
        ))}
      </div>
    </div>
  )
